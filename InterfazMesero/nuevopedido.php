...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Validación para asegurarse de que al menos un producto esté seleccionado
    if (empty($_POST['producto']) || in_array('', $_POST['producto'])) {
        $error = "Por favor, selecciona un producto.";
        echo "<p class='error-message'>Seleccione un producto, por favor.</p>";
        exit;
    }
    // Validación para las cantidades
    ...
    try {
        ...
        if ($result) {
            ...
            // Fila predeterminada si no hay detalles de pedido
            echo "<tr>";
            echo "<td data-label='Cantidad'><input type='number' name='cantidad[]' value='' class='cantidad-input'></td>";
            echo "<td data-label='Producto'>
                <select name='producto[]' class='producto-select'>
                    <option value=''>Seleccionar producto</option>";
            foreach ($productos as $producto) {
                echo "<option value='" . htmlspecialchars($producto['idProducto']) . "' data-precio='" . htmlspecialchars($producto['Precio']) . "'>" . htmlspecialchars($producto['Nombre']) . "</option>";
            }
            echo "</select>
                </td>";
            echo "<td data-label='Notas'><input type='text' name='notas[]'></td>";
            echo "<td data-label='Precio Unitario' class='precio-unitario'></td>";
            echo "<td data-label='Acciones' class='actions'>";
            echo '<button class="boton-cancelar" type="button">X Cancelar</button>';
            echo "</td>";
            echo "<input type='hidden' name='detalle_pedido_id[]' value='0'>";
            echo "</tr>";
        }
        echo "</tbody></table>";
